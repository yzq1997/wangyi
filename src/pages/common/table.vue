        ...
        <a-locale-provider :locale="zhCN">
            ...
            >   
                ...
                <span slot="optionaction" slot-scope="text,record">
                    ...
                    <slot name="comment" :record="record"></slot>
                    <slot name="status" :record="record"></slot>
                    <a href="javascript:;" class="tips" v-for="(item,index) in Array.isArray(option) && option.length && option.filter(item=>{return item['type'] && item['type']!='comment'})" :key="index" @click="item.fn(record,index)">
                        <!-- {{ item.text == 1?"编辑":(record.OrderIndex==1?"取消置顶":"置顶")}} -->
                        <!-- {{ item.text == 1?"查看":(item.text == 2?"删除":(record.sort==1?"取消置顶":"置顶"))}} -->
                        {{ item.text == 1?"查看":(item.text == 2?"删除":( item.text==3?(record.sort > 0?"取消置顶":"置顶"):( item.text==4?(record.sort > 0?"上移":""):(record.sort > 0?"下移":"") ) ))}}
                        <!-- {{index+1 == option.length?"":" | "}} -->
                    </a>
                    <slot name="restPassword" :record="record"></slot>
                    <slot name="more" :record="record"></slot>
